...
// foreach ($P_data as $data) {
    ...
    foreach ($S_list as $S_data) {
        ...
        if ($data[$sale_col] == $S_data['sale_value']) {
            ...
            /* 修改為特價價格 */
            if ($data['S_discount'] >= $S_data['S_discount']) {
                $data['S_discount'] = $S_data['S_discount'];
                $data['S_price'] = round($data['S_discount']*$data['P_price']);
            }
        }
        $data['P_price'] = round($data['P_price']);
    }
// }


// 判斷是否登入，是則加入cart資料庫，否則加入$_SESSION['cart_PID']['cart_qty']
if (isset($_GET['buy']) and $_GET['buy'] != "") {
    // 未登入，將資料寫入SESSION
    if ((!isset($_SESSION["login"])) || ($_SESSION["login"] == false)) {
        $_SESSION['cart_PID'][] = $_GET['P_ID'];
...
?>
    ...
    <?php
        ...
        if ($cart_rows > 0) {
            ...
            //若無產品，即新增一筆資料 
            $insertSQL = sprintf("INSERT INTO cart (UID, P_ID, qty) VALUES ('%d','%d','%d')", $UID, $P_ID, $qty);
            $result = mysqli_query($link, $insertSQL);
        }
    ?>
        ...
        <div id="main">
            ...
            <div class="productBook">
                ...
                <div class="productContent1">
                    ...
                    <!-- 定價與優惠價 -->
                    <?php 
                    if ($data['S_discount'] != 1) { 
                        echo "定價：".$data['P_price']."元<br>";
                        echo "優惠價：<span class='red'>".($data['S_discount']*10)."</span> 折 <span class='red'>";
                        echo $data['S_price']."</span> 元 <br>";
                        echo "<span class='bgred'>優惠活動：";
                        foreach($data['S_name'] as $name){
                            echo $name."<br></span>";
                        }
                    }else{
                        echo "定價：<span class='red'>".$data['P_price']."</span>元<br>";
                    }?>
                    運送方式：限定宅配 <br>
                </div>
                ...
                <form action="product.php" method="GET" enctype="multipart/form-data" name="buy">
                    <center>
                        <div class="shoppingCart">
                            <div>
                                <img src="<?php echo $root_cust; ?>/results/product/images/shopping_cart.png"><br>庫存><span>10</span>
                            </div>
                            <br>
                            ...
                            <input class="cartButton1" type="submit" value="放入購物車" onclick="">
